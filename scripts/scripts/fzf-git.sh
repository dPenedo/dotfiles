...
function gaa() {
  ...
  local -r header=$(
    cat <<-EOF
		...
		> CTRL_T for status preview | CTRL-F for diff preview | CTRL-X for blame preview
  ...
  eval "$git_unstaged_files" | fzf \
    ...
    --bind='ctrl-x:change-preview-label([ Blame ])' \
    --bind='ctrl-x:+change-preview(git blame --color-by-age {})' \
